...
void main() {
  ...
  // Feather the canvas borders so the glow dissolves into the background
  // instead of being clipped by a hard rectangular edge.
  vec2 uv = gl_FragCoord.xy / uResolution.xy;
  vec2 ef = abs(uv - 0.5) * 2.0;
  float edge = (1.0 - smoothstep(0.55, 1.0, ef.x)) * (1.0 - smoothstep(0.55, 1.0, ef.y));

  gl_FragColor = vec4(c, max(c.r, max(c.g, c.b)) * uOpacity * edge);
}
`;
